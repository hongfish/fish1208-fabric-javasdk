...
public class OrgManager {
     ...
     *
     * @param username         用户名
     * @param sk          带有节点签名密钥的PEM文件——sk内容
     * @param certificate 带有节点的X.509证书的PEM文件——certificate内容
     *
     * @return self
     */
    public OrgManager setUser(@Nonnull String leagueName, @Nonnull String orgName, @Nonnull String peerName, @Nonnull String username, @Nonnull String sk, @Nonnull String certificate) {
        IntermediateUser user = new IntermediateUser(leagueName, orgName, peerName, username, sk, certificate);
    ...
    public void addPeer(String peerName, String peerLocation, String peerEventHubLocation, String serverCrtPath, String clientCertPath, String clientKeyPath) {
        orgMap.get(cc).addPeer(peerName, String.format("%s%s", "grpc://", peerLocation), StrUtil.isNotBlank(peerEventHubLocation) ? String.format("%s%s", "grpc://", peerEventHubLocation) : null, serverCrtPath, clientCertPath, clientKeyPath);
    }
    ...
    public void add() {
        ...
        for (int i = 0; i < orgMap.get(cc).getPeers().size(); i++) {
            ...
            if (null != orgMap.get(cc).getPeers().get(i).getPeerEventHubLocation() && !orgMap.get(cc).getPeers().get(i).getPeerEventHubLocation().isEmpty()) {
